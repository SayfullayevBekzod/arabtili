...
def seed_enhanced_scenarios():
    # 1. Categories
    cat_daily, _ = ScenarioCategory.objects.get_or_create(name="Kundalik muloqot", icon="fas fa-home", order=1)
    cat_travel, _ = ScenarioCategory.objects.get_or_create(name="Sayohat", icon="fas fa-plane", order=2)
    cat_shop, _ = ScenarioCategory.objects.get_or_create(name="Bozor va Xarid", icon="fas fa-shopping-bag", order=3)
    ...
    # 3. Dialog Lines for Airport (Vocalized)
    DialogLine.objects.all().filter(scenario=sc_airport).delete()
    lines_airport = [
    ...
    DialogLine.objects.all().filter(scenario=sc_dinner).delete()
    lines_dinner = [
    ...
    # 5. Phrasebook Entries (Vocalized)
    phrases = [
    ...
    print("Enhanced conversational data seeded successfully with full diacritics!")

if __name__ == "__main__":
    seed_enhanced_scenarios()
